...
async fn update_article(
    ...
    .on_constraint("article_slug_key", |_| {
        Error::unprocessable_entity([(
            "slug",
            format!("duplicate article slug: {:?}", new_slug),
        )])
    })?
    .into_article();
...
async fn favorite_article(
    ...
    Ok(Json(ArticleBody {
        article: article_by_id(&state.db, auth_user.user_id, article_id).await?,
    }))
}
...
async fn get_tags(state: State<ApiContext>) -> Result<Json<TagsBody>> {
